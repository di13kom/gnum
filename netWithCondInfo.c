...
gpointer net_UdpBroadcaster(gpointer data)
{
	...
	RanVAl = g_random_double_range(5.0, 10.0);
	...
	g_sprintf(Buffer,"Hello");
	for(;;)
	{
		g_usleep(RanVAl * 1000000);
	...
	for(;;)
	{
					...
					net_CompareFunc) == NULL)
		{
			...
			g_printf("\033[0;32m UdpServer:\033[0m Recieved message \"%s\" of %"G_GSSIZE_FORMAT" bytes from %s\n", 
					Buffer, Bytes, g_inet_address_to_string(ClientAddress));
#endif
			if(net_ClientFactory((gpointer)ClientAddress) != NULL)
			{
				//g_object_ref(ClientAddress);
				TmpList = ListOfUsedAddresses;
				ListOfUsedAddresses = g_list_prepend(ListOfUsedAddresses,
						g_inet_address_to_string(ClientAddress));
		...
		else
		{
			g_mutex_unlock(&SharedMutex);
#ifdef DEBUG
		g_printf("mutex received in UdpReciever with escape\n");
#endif
#ifdef DEBUG
			//g_printf("\033[0;32m UdpServer:\033[0m Already Used Address\n");
#endif
		}
...
gpointer net_TcpReciever(gpointer data)
{
	...
	for(;;)
	{
		...
		g_mutex_lock(&SharedMutex);
#ifdef DEBUG
		g_printf("mutex locked in TcpReciever\n");
#endif
		if(g_list_find_custom(ListOfUsedAddresses,
					g_inet_address_to_string(ClientAddress),
					net_CompareFunc) == NULL)
		{
			if(net_TcpResponder((gpointer)IncomingSocket) != NULL)
			{
				//g_object_ref(ClientAddress);
				TmpList = ListOfUsedAddresses;
				ListOfUsedAddresses = g_list_prepend(ListOfUsedAddresses,
...
#ifdef DEBUG
				g_printf("mutex received in TcpReciever with additition\n");
#endif
			}
			g_mutex_unlock(&SharedMutex);
...
GSocket* net_CreateSocketAndBind(GSocketType SockType)
{
	...
	//address
	if((Address = g_inet_socket_address_new(g_inet_address_new_any(G_SOCKET_FAMILY_IPV4),PORT)) == NULL)
	{
#ifdef DEBUG
		g_printf("no parsable address\n");
#endif
...
gpointer net_ClientFactory(gpointer Address)
{
...
int net_StartPoint()
{
	g_printf("condition G_IO_HUP: %d\n", G_IO_HUP);
	g_printf("condition G_IO_IN: %d\n", G_IO_IN);
	g_printf("condition G_IO_NVAL: %d\n", G_IO_NVAL);
	g_printf("condition G_IO_ERR: %d\n", G_IO_ERR);
	g_printf("condition G_IO_OUT: %d\n", G_IO_OUT);
	g_printf("condition G_IO_PRI: %d\n", G_IO_PRI);
	GError *Error = NULL;
	GThread *Thread[3];
...
void net_RecieveMessage(GSocket* NewSocket, gchar* Data)
{
	...
	*/
	...
	printf("\033[0;35m Tcp: \033[0m Recieved message \"%s\": %"G_GSSIZE_FORMAT" bytes from server\n", Buffer, Bytes);
#endif
}
...
gboolean SocketCallback(GSocket *socket, GIOCondition cond, gpointer user_data)
{
	...
	{
		if((cond ^ G_IO_ERR) < cond)
		{
			g_printf("G_IO_ERR\n");
			g_printf("%d\n",(cond ^ G_IO_ERR));
		}
		if((cond ^ G_IO_HUP) < cond)
		{
			g_printf("G_IO_HUP\n");
			g_printf("%d\n",(cond ^ G_IO_HUP));
		}
		if((cond ^ G_IO_NVAL) < cond)
		{
			g_printf("G_IO_NVAL\n");
			g_printf("%d\n",(cond ^ G_IO_NVAL));
		}
